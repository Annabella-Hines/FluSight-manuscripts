...
dashboard_r_code <- paste0("C:/Users/",Sys.info()["user"],"/Desktop/GitHub/FluSight-manuscripts/ED_Visits_2025_26")
flusight_forecast_data <- paste0("C:/Users/",Sys.info()["user"],"/Desktop/GitHub/Flusight-forecast-hub")

source(paste0(dashboard_r_code,"/ed_functions2025-2026.R"))
...
model_metadata3 <- read.csv(paste0(dashboard_r_code, "/model_types.csv")) %>% 
...
raw_data <- raw_forecasts %>% 
  ...
  dplyr::mutate(quantile = as.numeric(quantile),
              prediction=prediction*100) #%>% #dplyr::filter(reference_date <= as.Date("2025-05-31")) %>% ## May 31 was the last submission date
...
supplemental_plt <- ggplot(ntl_ens_forecasts_all, aes(x = target_end_date, y = quantile_0.5))+
  geom_ribbon(aes(ymin = quantile_0.025, ymax = quantile_0.975, group = interaction(model, forecast_date),  alpha = "95%"),fill = "#3BBBB0")+
  geom_ribbon(aes(ymin = quantile_0.25, ymax = quantile_0.75, group = interaction(model, forecast_date), alpha = "50%"), fill = "#3BBBB0")+
  geom_line(aes(color = "Forecasted",group = interaction(model, forecast_date)))+
  geom_point(aes(color = "Forecasted", group = interaction(model, forecast_date)))+
...
ntl_ens_output <- ntl_ens_forecasts %>% 
  ...
  right_join(obs_data25_US, join_by(location, location_name, target_end_date)) %>% 
...
supplemental_ntl_ens_output <- ntl_ens_forecasts_all %>% 
  mutate(median = quantile_0.5,
         lower_50 = quantile_0.25, 
         upper_50 = quantile_0.75, 
         lower_95 = quantile_0.025,
...
supplemental_ntl_ens_output <- supplemental_ntl_ens_output %>%
  mutate(median = if_else(is.na(median),
                          "Intentionally left blank for visualization purposes", ##text needed for correct visualization on webpage
                          as.character(median)))

#write.csv(supplemental_ntl_ens_output, paste0(dashboard_r_code, "/output_data/supplementalfigure1_data.csv"), row.names = FALSE)
...
coverage50_flusight <- coverage50_states %>% filter(model == "FluSight-ensemble") 
coverage50_not_flusight <- coverage50_states %>% filter(model != "FluSight-ensemble") 

coverage_labels <- as_labeller(c(`0 wk inc flu prop ed visits` = "0 Week Ahead",
...
coverage_50_plt <-
  ggplot(coverage50_flusight, aes(x = target_end_date, 
                                  y = coverage50, group = model,
                                  col = model)) +
  geom_line(data = coverage50_not_flusight, aes(x = target_end_date, y = coverage50, group = model, color = "model")) + 
  geom_line(linewidth = 1) + geom_point(size = 2) +
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "black")+
  labs(y = "50% Coverage", x = "", color = "Model") +
  scale_color_manual(values = c("#00a599", adjustcolor("grey50", .35)),
                     labels = c("FluSight-ensemble", "Contributed Models"),
                     drop = FALSE) +
  theme_bw()+
  scale_x_date(breaks = seq.Date(from = min(coverage50_flusight$target_end_date), to= max(coverage50_flusight$target_end_date), by = "2 weeks"), 
               date_labels = "%d %b") +
  theme(axis.text.x = element_text(angle = 60, hjust = 1), panel.grid = element_blank(), 
        legend.position = "right")+
  facet_wrap(facets = vars(target), labeller = coverage_labels, scales = "free_x")
...
# Average log-scale WIS over time, excluding US
wis_time_log <- raw_scores25_log %>%
  filter(
    location != "US",
    model %in% include25$model
  ) %>%
  ...
  mutate(
    model_group = ifelse(model == "FluSight-ensemble", "FluSight-ensemble", "Other models")
  )


WIS_Season <- WIS_Season25 %>%
  rename(
    target_end_date = date,
    WIS = wis
  )

wis_states <- WIS_Season %>%
...
wis_flusight <- wis_states %>%
  filter(model == "FluSight-ensemble")

wis_not_flusight <- wis_states %>%
  filter(model != "FluSight-ensemble")

horizon_labels <- as_labeller(c(
...
wis_plt_facet <-
  ...
  scale_color_manual(
    values = c("#00a599", adjustcolor("grey50", .35)),
    labels = c("FluSight-ensemble", "Contributed Models"),
    drop = FALSE
  ) +
  theme_bw() +
  scale_x_date(
    breaks = seq.Date(
      from = min(wis_flusight$target_end_date),
      to = max(wis_flusight$target_end_date),
      by = "2 weeks"
    ),
    date_labels = "%d %b"
  ) +
...
wis_all_flusight <- wis_all_states %>%
  filter(model == "FluSight-ensemble")

wis_all_not_flusight <- wis_all_states %>%
  filter(model != "FluSight-ensemble")
...
wis_plt <-
  ...
  scale_x_date(
    breaks = seq.Date(
      from = min(wis_all_flusight$target_end_date),
      to = max(wis_all_flusight$target_end_date),
      by = "2 weeks"
    ),
    date_labels = "%d %b"
  ) +
