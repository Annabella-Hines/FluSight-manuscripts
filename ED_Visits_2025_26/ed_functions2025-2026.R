###ED FluSight 2025/26 Functions


dat_for_scores_function_ed <- function(all_dat, obs_data, includedf){
  ...
  dat_for_scores = all_dat %>% 
    ...
    mutate(
      report = ifelse(grepl("wk inc flu prop ed visits",target),
                      report)) %>% 
...
dat_for_scores_function_log_ed <- function(all_dat, obs_data, includedf){
  ...
  dat_for_scores = all_dat %>% 
    ...
    left_join( obs_data, 
               by=c("target_end_date", "location_name", "location"))%>%
    mutate(
      logreport = ifelse(grepl("wk inc flu prop ed visits",target),
                         logreport)) %>% 
    rename(date = target_end_date) %>% 
