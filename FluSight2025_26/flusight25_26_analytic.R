...
dashboard_r_code <- paste0("C:/Users/",Sys.info()["user"],"/Desktop/GitHub/FluSight-manuscripts/FluSight2025_26")
flusight_forecast_data <- paste0("C:/Users/",Sys.info()["user"],"/Desktop/GitHub/Flusight-forecast-hub")

source(paste0(dashboard_r_code,"/functions2025-2026.R"))
...
model_metadata <- read.csv(paste0(dashboard_r_code, "/model_types_2526.csv")) %>%
  mutate(model_name = paste(team_abbr, model_abbr, sep = "-")) %>% 
  ...
  mutate(mechanistic = case_when(model_name == "UNC_IDD-InfluPaint" ~ FALSE,
         ...
         stat = case_when(model_name == "CEPH-Rtrend_fluH" ~ TRUE, 
                          model_name == "FluSight-ensemble" ~ TRUE,
                          model_name == "FluSight-lop_norm" ~ TRUE,
                          TRUE ~ stat), 
...
raw_forecasts <- hub_con %>%
  dplyr::filter(
    reference_date >= "2025-11-22", #first reference date 
    reference_date <= "2026-05-31", #think the last will be May 20?
    !model_id %in% c(
      "FluSight-base_seasonal",  #seasonal baseline
      #"FluSight-baseline", "FluSight-ensemble", "FluSight-lop_norm",
      "FluSight-trained_mean","FluSight-trained_med", "FluSight-HJudge", "FluSight-Mean-HJudge"),   #ensembles not included
...
dat_for_scores25 <- dat_for_scores_function(all_dat25, obs_data25, include25) 
dat_for_scores25_log <- dat_for_scores_function_log(all_dat25, obs_data25, include25) 
...
table1 <- inc.rankings_all %>% 
  left_join(model_metadata, join_by("model" == "model_name")) %>% 
  filter(model!="FluSight-HJudge_ensemble") %>% 
  mutate(model_type = ifelse(ensemble_of_models == TRUE, paste("ens, ", model_type), model_type),
         display_name = paste0(model," (", toupper(model_type),")")) %>% 
  rename(Model = display_name,
         `Absolute WIS` = wis,
...
coverage_labels <- as_labeller(c(
  `0 wk inc flu hosp` = "0 Week Ahead",
  `1 wk inc flu hosp` = "1 Week Ahead", 
  `2 wk inc flu hosp` = "2 Week Ahead",
  `3 wk inc flu hosp` = "3 Week Ahead"
))
...
coverage_50_plt <-
  ...
  labs(y = "50% Coverage", x = "", color = "Model", title="50% Hospital Admissions Coverage by Model") +
  scale_color_manual(
    values = c(
...
coverage_95_plt <-
  ...
  labs(y = "95% Coverage", x = "", color = "Model", title="95% Hospital Admissions Coverage by Model") +
  scale_color_manual(
    values = c(
      "FluSight-ensemble" = "#00a599",
      "FluSight-baseline" = "#9b007e",
...
wis_plt <-
  ...
  labs(
    ...
    x = "",
    color = "Model",
    title = "Log rWIS by Week for Hospital Admissions Forecasts"
  ) +
  scale_color_manual(
    values = c(
      "FluSight-ensemble" = "#00a599",
