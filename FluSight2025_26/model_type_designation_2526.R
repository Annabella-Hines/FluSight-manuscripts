...
###This was run in April 2026 and if run again, may pull in models that were updated for the 2024/2025 season because the same FluSight forecast hub repository was used for both seasons.
mylib <- getwd()
...
df <- df %>% 
  mutate(methods = tolower(methods_long),
...
write.csv(df, paste0("C:/Users/", Sys.info()["user"], "/Desktop/GitHub/FluSight-manuscripts/FluSight2025_26/model_types_2526.csv"), row.names = FALSE)
